...
@router.message(or_f(Command("basketball"), F.text.casefold().startswith("баскетбол")))
async def basketball_handler(message: Message):
    ...
    if len(args) != 2:
        await message.answer(
            ...
            f"<code>·····················</code>\n"
            f"🏀 /basketball [ставка]\n\n"
            f"Пример: /basketball 100\n"
            f"Пример: баскетбол все",
    ...
    dice_msg = await message.answer_dice(emoji=DiceEmoji.BASKETBALL)
    ...
    if value in (1, 2):
        ...
        coeff = 1.5
        outcome = "На грани фола"
        emoji_res = "🏀"
        win = int(amount * coeff)
    else:  # value == 5
        coeff = 2
        outcome = "Идеальное попадание"
        emoji_res = "🎉"
        win = int(amount * coeff)
    ...
    await db.execute(
        ...
        (user_id, "basketball", amount, status, f"value={value}, coeff={coeff}")
    )
